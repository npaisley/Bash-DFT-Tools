...
log_write () { #####make it so that it sets a version and udates if teh version is old
mkdir ${LOG_DIR}
touch ${LOG}
cat > ${LOG} <<EOF 
...
while getopts "f:t:s:vTEhr" opt; do 
	case "${opt}" in
